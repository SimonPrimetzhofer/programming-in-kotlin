...
abstract sealed class FList<out E> {
    ...
    tailrec fun forEach(action: (E) -> Unit) {
        when (this) {
            is Cons -> {
                action(head)
                tail.forEach(action)
...
fun main() {
    ...
    fun printFirstNegative(ns: FList<Int>) : Unit {
        ns.iterate (
            fun (n : Int)  {
                if (n < 0) {
                    println(n)
                    return@printFirstNegative
                }
            }
        )
    }

    fun printAllNegatives(ns: FList<Int>) : Unit {
        ns.iterate lambda@{
                n : Int ->
            run {
                if (n < 0) {
