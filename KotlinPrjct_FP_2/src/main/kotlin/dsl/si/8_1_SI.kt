...
data class Velocity(val v : Double, val u: VelUnit) : Quantity<VelUnit>(v, u)  {
    ...
    operator fun times(d : Duration) : Length {
         if (unit == KmPerHour) {
             return Length(this.inKMpH * d.inH, Kilometer)
         } else {
             return Length(this.inMpS * d.inSec, Meter)
         }
    }
...
fun main() {
    ...
    val cm1mm4 = 4.mm + 1.cm
    println("cm1mm4 = $cm1mm4")
    ...
    val velHalfM = halfMarathon / halfMTime
    println("Vel for a half = ${velHalfM.asKMpH}")

    val timeFull = marathon / velHalfM
    println("Time for a full = ${timeFull.inMin} min")
    println("Time for a full = ${timeFull.inH} h")

    val distFull = velHalfM * timeFull
    println("Distance after $timeFull = $distFull")
